...
class CptnCat < Chingu::Window
  ...
  def initialize
    ...
    @map = Map.new(self, "media/cpn/map.txt")
    @cptn = CptnRuby.new(self, 100, 100)
    # Scrolling is stored as the position of the top left corner of the screen.
    @screen_x = @screen_y = 0
    Sound["/media/cpn/boing1.ogg"] # cache sound by accessing it once
    Sound["media/cpn/boing2.ogg"]
    Sound["media/cpn/chime2.ogg"]
    Sound["media/cpn/meow.ogg"]
    Sound["media/cpn/crush.ogg"]
    Sound["media/cpn/intro.ogg"].play(0.6) # Intro Music
  end
  def update
    move_x = 0
  ...
  def button_down(id)
    if id == Button::Kb2 then self.second_player end
...
class CptnRuby < Chingu::GameObject
  ...
  def initialize(window, x, y)
    ...
    @standing, @walk1, @walk2, @jump, @wall_climb, @blink=
      *Image.load_tiles(window, "media/cpn/cpncat.png", 50, 50, false)
...
#      puts (milliseconds / 175 % 4)
  ...
  def try_to_jump
    if not would_fit(0, 1) then
      ...
      if @boing == true
        Sound["media/cpn/boing1.ogg"].play(0.4)
        @boing = false
      else
        Sound["media/cpn/boing2.ogg"].play(0.4)
        @boing = true
      end
    end    
    if @wall != 0
...
#    Sound["media/chime2.ogg"].play(0.5)
    if rand(6) == 1
      Sound["media/cpn/meow.ogg"].play(1.0)
      Sound["media/cpn/chime2.ogg"].play(0.3)
    else
      Sound["media/cpn/chime2.ogg"].play(0.5)
    end
  end

  def crush     # wall-climb sound effect
    if @cooling_down >= 26
      Sound["media/cpn/crush.ogg"].play(0.5)
      @cooling_down = 0
    else
      @cooling_down += 1
    end
...
class Map < Chingu::GameObject
  ...
  def initialize(window, filename)
    ...
    @tileset = Image.load_tiles(window, "media/cpn/tileset.png", 60, 55, true)
    @sky = Image.new(window, "media/cpn/sky.png", true)

    gem_img = Image.new(window, "media/cpn/gem.png", false)
    @gems = []
...
class DustParticle < Chingu::GameObject
  ...
  def initialize(window, x, y, life)
    @x = x
    @y = y
    @graphic = Image.new(window, "media/cpn/dust.png", false)
    @lifespan = life
