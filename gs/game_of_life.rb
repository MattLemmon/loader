# game_of_life.rb - Classes GameOfLife
...
class GameOfLife < Chingu::GameState
  ...
  def initialize
    ...
    self.input={  :holding_space => :update_grid,
                  :left_mouse_button => :start_painting, 
                  :released_left_mouse_button => :stop_painting,
                  :right_mouse_button => :start_erasing,
                  :released_right_mouse_button => :stop_erasing,
                  :r => :reset,
                  [:w, :s ] => :toggle_running,
                  [:a, :mouse_wheel_up] => :prev_pattern,
                  [:d, :mouse_wheel_down] => :next_pattern,
                  :p => Pause
                }
    ...
    @pattern_info = Chingu::Text.create(:x => 1, :y => 1, :size => 24, :color => Gosu::Color::BLACK)
    update_pattern_info
  end 
  ...
  def update_pattern_info
    @pattern_info.text = "Current pattern: #{@pattern}. Left-click with mouse to paint."
  end
  ...
  def update
    ...
    $window.caption = "Conway Generation #{@@tick}.     Use the mouse, click to paint.        Controls:   A  W  S  D   spacebar   mouse"
  end

  def draw
    super
    fill_rect([0,0,900,700], 0xffffffff, -2)
    draw_grid
